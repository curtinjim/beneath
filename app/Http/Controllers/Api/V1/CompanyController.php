...
class CompanyController extends Controller {
    public function index(Request $request): AnonymousResourceCollection {
        $actors = Actor::where('actor_type', 'organisation')
    ...
    public function show(string $id): JsonResponse {
        return (new ActorResource(Actor::where('actor_type','organisation')->findOrFail($id)))->response();
    }
    public function update(ActorRequest $request, string $id): JsonResponse {
        $actor = Actor::where('actor_type','organisation')->findOrFail($id);
        $actor->update($request->validated());
        return (new ActorResource($actor))->response();
    }
    public function destroy(string $id): JsonResponse {
        Actor::where('actor_type','organisation')->findOrFail($id)->delete();
