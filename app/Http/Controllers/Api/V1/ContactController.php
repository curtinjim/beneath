...
class ContactController extends Controller {
    public function index(Request $request): AnonymousResourceCollection {
        $actors = Actor::where('actor_type', 'person')
            ->when($request->search, fn($q, $s) => $q->where('display_name', 'like', "%$s%"))
            ->when($request->importance_tier, fn($q, $t) => $q->where('importance_tier', $t))
    ...
    public function show(string $id): JsonResponse {
        return (new ActorResource(Actor::where('actor_type','person')->findOrFail($id)))->response();
    }
    public function update(ActorRequest $request, string $id): JsonResponse {
        $actor = Actor::where('actor_type','person')->findOrFail($id);
        $actor->update($request->validated());
        return (new ActorResource($actor))->response();
    }
    public function destroy(string $id): JsonResponse {
        Actor::where('actor_type','person')->findOrFail($id)->delete();
